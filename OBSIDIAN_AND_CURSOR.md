...
```bash
git submodule update --init Planning-Sprint
```
...
1. Open **Obsidian** → **Open folder as vault** → select the **Planning-Sprint** folder (e.g. `OASIS_CLEAN/Planning-Sprint`).
...
- Open **OASIS_CLEAN** in Cursor (File → Open Folder). You’ll see **Planning-Sprint** and its contents.
- Edit any file under `Planning-Sprint/`; use `@Planning-Sprint/filename.md` in chat so the AI can read/edit notes.
- **To commit/push Planning-Sprint changes:** either use Obsidian Git, or in a terminal:
  ...
  git push
  ```
  That pushes to the Planning-Sprint repo. Then in the repo root: `git add Planning-Sprint && git commit -m "Update Planning-Sprint submodule" && git push` to record the new submodule commit in OASIS_CLEAN (optional but recommended so others get the same pointer).

---

## 4. Team workflow

- **OASIS_CLEAN:** Clone as usual, then run `git submodule update --init Planning-Sprint`.
